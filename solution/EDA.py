...
## final fig ##
###############
SARIMAX_submission = pd.read_csv(f'{base.INPUT_DIR}/submission_files/SARIMAX_incbac_submission.csv')
TBATS_submission = pd.read_csv(f'{base.INPUT_DIR}/submission_files/TBATS_incbac_submission.csv')
DHR_submission = pd.read_csv(f'{base.INPUT_DIR}/submission_files/DHR_incbac_submission.csv')
...
DHR = DHR_submission[DHR_submission['id'].str.contains("validation")].iloc[:, 1:].sum().values

result_final = pd.DataFrame({'Actual':base.no_filter()[-28:], 'SARIMAX Forecasts':SARIMAX,'DHR Forecasts': DHR, 'TBATS Forecasts':TBATS})

result_final.plot()
plt.savefig(f'{base.INPUT_DIR}/images/final_fig.png', format="png", bbox_inches="tight", dpi = 300)

##############
## Acurácia ##
##############
print('MASE SARIMAX:', np.round(base.MASE(base.no_filter()[0:-28], base.no_filter()[-28:], SARIMAX), 3))
print('MASE DHR:', np.round(base.MASE(base.no_filter()[0:-28], base.no_filter()[-28:], DHR), 3))
...
print('sMAPE DHR:', round(base.calculate_smape(base.no_filter()[-28:], DHR)/100, 3))
