#######################################################
# Note: 44h to run without pre trained models         #
# Especificacoes: intel core i5 7th gen; DDR4 8gb RAM # 
#######################################################
        ...
        for index, id in enumerate(tqdm(base.sales_train['id'])):
            ...
            if iter == '_validation':
                ...
                if os.path.exists(f'{base.INPUT_DIR}/DHR_models/{dept}_{store}_validation.pkl'):
                    ...
                    dhr_forecast_valid = result_dhr_valid.predict(28, X= pd.concat([exp_var[1913:1941], pd.DataFrame(x_f[-28:])\
                                                                                    .set_index(exp_var[1913:1941].index)], axis = 1))
                    ...
                    result_dhr_valid = pm.auto_arima(\
                        serie_valid, d = 1, start_p=0, start_q=0, max_p=3, max_q=3,\
                            seasonal = True, D=1, start_P=0, start_Q=0, max_P=1, max_Q=1,\
                                information_criterion='aic', error_action='ignore', stepwise=True,\
                ...
                SARIMAX_submission.iloc[index_, 1:] = disaggregated_sarimax
                DHR_submission.iloc[index_, 1:] = disaggregated_dhr
                TBATS_submission.iloc[index_, 1:] = disaggregated_tbats
                ...
                if os.path.exists(f'{base.INPUT_DIR}/DHR_models/{dept}_{store}_evaluation.pkl'):
                    ...
                    dhr_forecast_eva = result_dhr_eva.predict(28, X= pd.concat([exp_var[1941:], pd.DataFrame(x_f[-28:])\
                                                                                .set_index(exp_var[1941:].index)], axis = 1))
                ...
                ########################
                index_ = base.sample_submission[base.sample_submission['id'] == id_].index[0]
                SARIMAX_submission.iloc[index_, 1:] = disaggregated_sarimax, 0
                DHR_submission.iloc[index_, 1:] = disaggregated_dhr, 0
                TBATS_submission.iloc[index_, 1:] = disaggregated_tbats
