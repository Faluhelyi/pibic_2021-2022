    ...
    while c <= 1:
        ...
        for index, id in enumerate(base.sales_train['id']):
            ...
            if iter == '_validation':
                ...
                #################################################################################################
                proportion = np.mean(base.filter_item_store(item, store)[-56:-28])/np.mean(base.filter_dept_store(dept, store)[-56:-28])
                ...
                ### Top-Down approach - top-down Gross-Sohl method F (Proportions of the historical averages) ###
                #################################################################################################
                proportion = np.mean(base.filter_item_store(item, store)[-28:])/np.mean(base.filter_dept_store(dept, store)[-28:])
