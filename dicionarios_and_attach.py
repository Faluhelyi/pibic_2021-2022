...
for store in stores:
    ...
    for departament in departaments:
        ...
        (sales_train_eva['store_id'] == store)].groupby(['store_id', 'dept_id'])[a].sum()), np.array([np.nan]*28)),
        axis = None)[0:1941]
...
for store in stores:
    ...
    for departament in departaments:
        ...
        (sales_train_val['store_id'] == store)].groupby(['store_id', 'dept_id'])[a].sum()), np.array([np.nan]*28)),
        axis = None)[0:1913]
